...
@section('title', 'Tableau de bord Admin - SEN DISTRIBUTION')
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-12 col-sm-6 col-xl-3">
            <div class="card border-0 shadow-sm rounded-4 p-3 h-100 bg-white">
                ...
                <div class="d-flex align-items-baseline mt-2">
                    <h2 class="fw-bold text-dark m-0 me-2">{{ $totalProducts ?? 245 }}</h2>
                </div>
        ...
        <div class="col-12 col-sm-6 col-xl-3">
            <div class="card border-0 shadow-sm rounded-4 p-3 h-100 bg-white">
                ...
                <div class="d-flex align-items-baseline mt-2">
                    <h2 class="fw-bold text-dark m-0 me-2">{{ $productsInStock ?? 128 }}</h2>
                </div>
        ...
        <div class="col-12 col-sm-6 col-xl-3">
            <div class="card border-0 shadow-sm rounded-4 p-3 h-100 bg-white">
                ...
                <div class="d-flex align-items-baseline mt-2">
                    <h2 class="fw-bold text-dark m-0 me-2" style="font-size: 1.6rem;">{{ number_format($totalSalesAmount ?? 8750000, 0, ',', ' ') }} FCFA</h2>
                </div>
        ...
        <div class="col-12 col-sm-6 col-xl-3">
            <div class="card border-0 shadow-sm rounded-4 p-3 h-100 bg-white">
                ...
                <div class="d-flex align-items-baseline mt-2">
                    <h2 class="fw-bold text-dark m-0 me-2">{{ $pendingOrders ?? 12 }}</h2>
                </div>
...
<div class="card border-0 shadow-sm rounded-4 bg-white p-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="{{ route('admin.sales.index') }}" class="text-primary text-decoration-none fw-semibold" style="font-size: 13px;">Voir tout</a>
    </div>
        ...
        <table class="table table-hover align-middle mb-0" style="font-size: 13px;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    {{-- Client : extraction propre du nom --}}
                    <td>
                        {{ is_object($sale->client) ? $sale->client->name : ($sale->client_name ?? 'Client comptoir') }}
                    </td>
    ...
    {{ number_format($sale->total_amount ?? $sale->total ?? 0, 0, ',', ' ') }} FCFA
</td>

                    {{-- Mode de paiement --}}
                    <td>
                        <span class="badge bg-info-subtle text-info px-2 py-1 rounded-2">
                            {{ $sale->payment_method ?? $sale->payment ?? 'Espèces' }}
                        </span>
                    </td>
